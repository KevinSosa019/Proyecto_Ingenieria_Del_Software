...
def client_order_detail(request, order_id):
    """Detalle de una orden del cliente."""
    user_id = request.session.get("user_id")
    if not user_id:
        return redirect("/accounts/login/")
    ...
    cur.execute("""
        SELECT 
            ...
            o.fecha_creacion, o.fecha_asignacion, 
            o.fecha_inicio, o.fecha_finalizacion,

            t.nombre, p.nombre, e.nombre,

            m.serial, m.modelo, m.ubicacion,

            u.primer_nombre + ' ' + u.primer_apellido,
            tec.primer_nombre + ' ' + tec.primer_apellido,
            tec.telefono
        FROM ordenes_trabajo o
        LEFT JOIN tipos_problema t ON o.tipo_id = t.id
        LEFT JOIN prioridades p ON o.prioridad_id = p.id
    ...
    order = {
        ...
        "tipo": row[7], "prioridad": row[8], "estado": row[9],
        "serial": row[10], "modelo": row[11], "location": row[12],
